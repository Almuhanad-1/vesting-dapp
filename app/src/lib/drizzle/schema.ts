...
export const vestingSchedules = pgTable('vesting_schedules', {
  ...
  releasedAmount: varchar('released_amount', { length: 78 }).default('0').notNull(),
...
export const batchDeployments = pgTable('batch_deployments', {
  id: uuid('id').defaultRandom().primaryKey(),
  creatorAddress: varchar('creator_address', { length: 42 }).notNull(),
  name: varchar('name', { length: 100 }),
  description: text('description'),
  tokenCount: integer('token_count').notNull(),
  totalVestingSchedules: integer('total_vesting_schedules').notNull(),
...
export const usersRelations = relations(users, ({ many }) => ({
  ...
  batchDeployments: many(batchDeployments),
}));

export const deployedTokensRelations = relations(deployedTokens, ({ one, many }) => ({
  owner: one(users, {
    fields: [deployedTokens.ownerAddress],
    references: [users.address],
  }),
  vestingSchedules: many(vestingSchedules),
  batchDeployment: one(batchDeployments, {
    fields: [deployedTokens.batchId],
    references: [batchDeployments.id],
  }),
}));

export const vestingSchedulesRelations = relations(vestingSchedules, ({ one, many }) => ({
...
export const batchDeploymentsRelations = relations(batchDeployments, ({ one, many }) => ({
  creator: one(users, {
    fields: [batchDeployments.creatorAddress],
    references: [users.address],
  }),
  tokens: many(deployedTokens),
}));
...
export type BatchDeployment = typeof batchDeployments.$inferSelect;
export type NewBatchDeployment = typeof batchDeployments.$inferInsert;
